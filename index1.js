...
app.post('/candidate', function(req, res) {
    ...
    // Check if the employee has been rated for the current day
    conn.query("SELECT * FROM manager WHERE Ename = ? ", [t1], function(err, results) 
        {
        if (err) {
            console.error('Error querying manager table: ' + err.stack);
            res.status(500).send('you are late you have been rated');
            return;
        }

        if (results.length > 0) {
            // Employee has already been rated for today, do not allow changes to the timesheet
            res.send("You have already been rated for today. You cannot make changes to your timesheet.");
        } else {
            // Employee has not been rated for today, insert the timesheet data
            conn.query("INSERT INTO employee (Ename, timesheet, work) VALUES (?, ?, ?)", [t1, t2, t3], function(err, result) {
                ...
                if (result.affectedRows > 0) {
                    ...
                    res.redirect('/submitted'); // Redirect to the submitted page
                }
            });
        }
    });
